...
namespace :hierarchy_spike do
  desc "Publish hard-coded Service Manual Guides to the publishing API"
  task publish_guides: [:environment] do
    publish_from_file("guides.yml")
  end

  desc "Publish hard-coded Service Manual Topic hierarchy to the publishing API"
  task publish_topics: [:environment] do
    publish_from_file("topic_hierarchy.yml")
  end
